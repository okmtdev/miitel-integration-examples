...
- **反映タイミング**: 外観設定はソフトフォンのパネルを**次に開いた時**に反映されます
  （表示中のパネルには即時反映されないことがあります）。初期表示から効かせたい場合は、
  本体（`v1.js`）読み込み前の `conf` 段階で指定するのが確実です。
- `color` の**既定は `black`（黒基調）**です。既定状態で「黒」を指定しても見た目は変わりません。
  効果を確認するには「白（`white`）」を指定して比較してください。
